...
import { CheckCircle, Circle, AlertCircle, Zap, Trophy, Plus, type LucideIcon } from 'lucide-react'
...
function DroppableColumn({ id, title, tasks, icon: Icon, count, onEdit, onDelete }: DroppableColumnProps) {
...
export default function ScrumBoard() {
  ...
  const sprintTasks = activeSprint 
    ? allTasks.filter((task: Task) => task.sprintId === activeSprint.id)
    : []
  ...
  const tasksByStatus = {
    todo: sprintTasks.filter((t: Task) => t.status === 'todo'),
    'in-progress': sprintTasks.filter((t: Task) => t.status === 'in-progress'),
    review: sprintTasks.filter((t: Task) => t.status === 'review'),
    done: sprintTasks.filter((t: Task) => t.status === 'done'),
  }

  const getTotalStoryPoints = (tasks: Task[]) => {
    return tasks.reduce((sum: number, task: Task) => sum + (task.storyPoints || 0), 0)
  }
  ...
  const handleDragStart = (event: any) => {
    const task = sprintTasks.find((t: Task) => t.id === event.active.id)
